...
try {
  // Bun.file().exists() always returns false for directories, so stat the path.
  const setupExists = await stat(setupDir)
    .then((entry) => entry.isDirectory())
    .catch(() => false);
  if (!setupExists) {
    console.log(chalk.gray('scripts/setup already removed, skipping.'));
    targetBDone = true;
  } else {
