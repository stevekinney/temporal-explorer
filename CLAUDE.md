...
| ----------------------------------- | ---------------- | -------------------------------------------------- |
| `apps/explorer`                     | Application      | SvelteKit local UI (Cinder, Svelte Flow, ELK)      |
| `packages/api`                      | Internal package | Public library surface re-exported by the npm root |
| `packages/cli`                      | Internal package | `temporal-explorer` command layer over the API     |
| `packages/schemas`                  | Internal package | Zod artifact schemas + JSON Schema emission        |
| `packages/analyzer`                 | Internal package | ts-morph static analysis                           |
| `packages/history`                  | Internal package | Event History parsing + live connections           |
| `packages/mapper`                   | Internal package | Source-to-runtime overlay + aggregate analysis     |
| `packages/renderers`                | Internal package | Markdown, Mermaid, and `.d.ts` renderers           |
| `packages/typescript-configuration` | Configuration    | Shared TypeScript compiler options                 |

Root-level directories: `fixtures/` holds real generated fixture projects
(sources, generated histories with provenance, and committed `.temporal-explorer`
artifacts); `scripts/` holds fixture generation, schema emission, benchmarks,
replay, release, and UI gate scripts; `examples/` holds runnable library usage
examples. Fixture histories are generated by executing real Workflows against
a Temporal test server (`bun run fixtures:generate-histories`), never
hand-written, and `bun run fixtures:regenerate-artifacts` is the only writer
of committed artifacts.
